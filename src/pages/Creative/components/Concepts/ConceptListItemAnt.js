...
import {Card, Col, Popconfirm, Tooltip} from 'antd';
import {DeleteOutlined, EditOutlined} from '@ant-design/icons';
import {FILE_DISPLAY_URL} from "../../../../constants";
import {InventoryFormats} from "../../../Container/constants";
...
function ConceptListItemAnt(props) {
  ...
  const PreviewMediaComponent = useCallback(() => {
    if(fileType === InventoryFormats.BANNER){
      return <img
        className="concept-image"
        alt={name} src={fileUUID
        ? `${FILE_DISPLAY_URL}/${fileUUID}`
        : noImage}
      />
  ...
  return (
    <Col>
      <Card style={{ width: 200 }}
        ...
        actions={[
          <Tooltip placement="top" title="Edit this concept">
            <EditOutlined style={{color: "#545cd8"}} key="edit" onClick={handleEdit} />
          </Tooltip>,
          <Popconfirm placement="top" title="Are you sure to delete this concept?" onConfirm={handleConfirmDelete} okText="Yes" cancelText="No">
            <Tooltip placement="top" title="Delete this concept">
      ...
      >
        <Meta title={
          <Tooltip title={name}>
            <span onClick={handleEdit} style={{ cursor: "pointer" }}>{name}</span>
          </Tooltip>}
        />
      </Card>
    </Col>
